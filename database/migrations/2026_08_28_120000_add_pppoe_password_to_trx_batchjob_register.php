...
return new class extends Migration
{
    ...
    public function up(): void
    {
        if (Schema::hasTable('trx_batchjob_register')) {
            if (!Schema::hasColumn('trx_batchjob_register', 'pppoe_password')) {
                try {
                    DB::statement("ALTER TABLE `trx_batchjob_register` ADD `pppoe_password` VARCHAR(50) NULL DEFAULT NULL AFTER `note_request`");
                } catch (\Throwable $e) {}
            }
        }
    ...
    public function down(): void
    {
        if (Schema::hasTable('trx_batchjob_register')) {
            if (Schema::hasColumn('trx_batchjob_register', 'pppoe_password')) {
                try {
                    DB::statement("ALTER TABLE `trx_batchjob_register` DROP COLUMN `pppoe_password`");
                } catch (\Throwable $e) {}
            }
        }
    }
