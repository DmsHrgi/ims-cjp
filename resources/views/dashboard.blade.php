...
@section('content')
    ...
    <div id="company-visualization-section" class="space-y-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:border-indigo-100 transition-colors">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs font-medium text-gray-400 uppercase tracking-wider">Total Perusahaan</span>
                    <span class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center text-xs">
                        <i class="fa-solid fa-building"></i>
                    </span>
                </div>
                <div class="flex items-baseline gap-1.5">
                    <span class="text-2xl font-black text-gray-800 tabular-nums">{{ number_format($totalPerusahaan, 0, ',', '.') }}</span>
                    <span class="text-xs font-medium text-gray-400">Entitas</span>
                </div>
                <p class="text-[11px] text-gray-400 mt-1.5 flex items-center gap-1">
                    ...
                    {{ $totalMultiSite }} Multi-Layanan / Cabang
                </p>
            </div>

            <div class="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:border-blue-100 transition-colors">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs font-medium text-gray-400 uppercase tracking-wider">Total ID Pelanggan</span>
                    <span class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center text-xs">
                        <i class="fa-solid fa-network-wired"></i>
                    </span>
                </div>
                <div class="flex items-baseline gap-1.5">
                    <span class="text-2xl font-black text-gray-800 tabular-nums">{{ number_format($totalSemuaLayanan, 0, ',', '.') }}</span>
                    <span class="text-xs font-medium text-gray-400">Titik Layanan</span>
                </div>
                <p class="text-[11px] text-gray-400 mt-1.5 flex items-center gap-1">
                    <i class="fa-solid fa-calculator text-blue-500 text-[10px]"></i>
                    Rata-rata {{ $avgLayananPerPerusahaan }} ID / Perusahaan
                </p>
            </div>

            <div class="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:border-emerald-100 transition-colors">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs font-medium text-gray-400 uppercase tracking-wider">Layanan Aktif</span>
                    <span class="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center text-xs">
                        <i class="fa-solid fa-circle-check"></i>
                    </span>
                </div>
                <div class="flex items-baseline gap-1.5">
                    <span class="text-2xl font-black text-emerald-600 tabular-nums">{{ number_format($totalAktifServices, 0, ',', '.') }}</span>
                    <span class="text-xs font-medium text-gray-400">Layanan</span>
                </div>
                <p class="text-[11px] text-gray-400 mt-1.5">
                    {{ $totalSemuaLayanan > 0 ? round(($totalAktifServices / $totalSemuaLayanan) * 100, 1) : 0 }}% dari total registrasi
                </p>
            </div>

            <div class="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:border-amber-100 transition-colors">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs font-medium text-gray-400 uppercase tracking-wider">Suspend & Terminasi</span>
                    <span class="w-8 h-8 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center text-xs">
                        <i class="fa-solid fa-triangle-exclamation"></i>
                    </span>
                </div>
                <div class="flex items-baseline gap-2">
                    <span class="text-2xl font-black text-amber-600 tabular-nums">{{ number_format($totalSuspendServices, 0, ',', '.') }}</span>
                    <span class="text-xs text-gray-400">Suspend</span>
                    <span class="text-gray-300">/</span>
                    <span class="text-2xl font-black text-rose-600 tabular-nums">{{ number_format($totalTerminasiServices, 0, ',', '.') }}</span>
                    <span class="text-xs text-gray-400">Terminasi</span>
                </div>
                <p class="text-[11px] text-gray-400 mt-1.5 flex items-center gap-1">
                    <i class="fa-solid fa-clock text-blue-400 text-[10px]"></i>
                    {{ $totalProsesServices }} ID dalam proses registrasi
                </p>
            </div>
        </div>
