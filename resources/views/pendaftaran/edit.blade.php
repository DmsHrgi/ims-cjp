...
@section('content')
    ...
    @php
        ...
        $rawTipeDb = $reg->tipe_pelanggan ?? ($pelanggan->tipe_pelanggan ?? ($data->tipe_pelanggan ?? null));
        $rawTipeLower = strtolower(trim((string)$rawTipeDb));
        if (in_array($rawTipeLower, ['lama', 'kuning', 'yellow'], true)) {
            $valTipePelanggan = 'lama';
        } elseif (in_array($rawTipeLower, ['baru', 'biru', 'blue'], true)) {
            $valTipePelanggan = 'baru';
        } else {
            $valTipePelanggan = '';
        }
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
        ...
        <form method="POST" action="{{ route('pendaftaran.update', $data->nomor_internet) }}" enctype="multipart/form-data" class="p-6 space-y-6" id="formEdit">
            ...
            <div id="sec-edit-informasi-pelanggan" class="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/80 shadow-xs space-y-5">
                ...
                <!-- Baris 5: Tipe Pelanggan & Warna Border Tabel (Opsional) -->
                <div class="p-3.5 bg-slate-50/80 rounded-xl border border-slate-200/70 space-y-2">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                        <label class="relative flex items-center gap-2.5 p-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 cursor-pointer transition-all shadow-2xs group has-[:checked]:border-slate-500 has-[:checked]:bg-slate-100 has-[:checked]:ring-2 has-[:checked]:ring-slate-300/40">
                            <input type="radio" name="tipe_pelanggan" value="" {{ old('tipe_pelanggan', $valTipePelanggan) == '' ? 'checked' : '' }} class="w-4 h-4 text-slate-600 focus:ring-slate-500 cursor-pointer">
            ...
            <div id="sec-edit-pasang" class="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/80 shadow-xs space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="jenis_bangunan" id="editJenisBangunanPasang" autocomplete="off" required placeholder="Contoh: RUMAH, RUKO, GEDUNG" value="{{ old('jenis_bangunan', $valJenisBangunanPasang) }}" class="w-full bg-white border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-slate-800 py-2.5 px-3.5 text-sm rounded-xl outline-none transition-all placeholder-slate-400">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1.5">No / Blok Bangunan <span class="text-rose-500 font-bold">*</span></label>
